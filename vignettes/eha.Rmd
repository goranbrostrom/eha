...
```{r coxph}
fit <- coxph(Surv(enter, exit, event) ~ sex + civ + imr.birth, data = oldmort)
summary(fit)
```
...
*piecewise constant hazard* function. It is a well-known fact that any 
continuous function can arbitrary well be approximated by a step function, 
simply by taking small enough steps.

An example: The data sets `swepop` and `swedeaths` contain age and sex specific 
yearly information on population size and number of deaths, respectively. They 
both cover the years 1968--2019.
...
The funny rownames and the column `id` is created by the function `reshape`, 
...
rownames(dat) <- 1:NROW(dat) # Simplify rownames to avoid distraction.
head(dat)
tail(dat)
```
...
1968--2019, or `r as.integer(round(sum(dat$deaths) / (2019 - 1967)))` deaths per year on average.

```{r tpchreg}
fit <- tpchreg(oe(deaths, pop) ~ sex + I(year - 2000), time = age, last = 105,
               data = dat)
summary(fit)
```
