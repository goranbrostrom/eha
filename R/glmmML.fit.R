...
#' 
#' This function is called by \code{glmmML}, but it can also be called directly
#' by the user.
#' 
#' In the optimisation, "vmmin" (in C code) is used.
#' 
#' @param X Design matrix of covariates.
#' @param Y Response vector. Or two-column matrix.
#' @param weights Case weights. Defaults to one.
#' @param cluster.weights Cluster weights. Defaults to one.
#' @param start.coef Starting values for the coefficients.
#' @param start.sigma Starting value for the mixing standard deviation.
#' @param fix.sigma Should sigma be fixed at start.sigma?
#' @param cluster The clustering variable.
#' @param offset The offset in the model.
#' @param family Family of distributions. Defaults to binomial with logit link.
#' Other possibilities are binomial with cloglog link and poisson with log
#' link.
#' @param method Laplace (1) or Gauss-hermite (0)?
#' @param n.points Number of points in the Gauss-Hermite quadrature. Default is
#' \code{n.points = 1}, which is equivalent to Laplace approximation.
#' @param control Control of the iterations. See \code{\link{glm.control}}.
#' @param intercept Logical. If TRUE, an intercept is fitted.
#' @param boot Integer. If > 0, bootstrapping with \code{boot} replicates.
#' @param prior Which prior distribution? 0 for "gaussian", 1 for "logistic", 2
#' for "cauchy".
#' @return A list. For details, see the code, and \code{glmmML}.
#' @author Göran Broström
#' @seealso \code{\link{glmmML}}, \code{glmmPQL} in the package \code{MASS},
#' and \code{lmer} in the package \code{lme4}.
